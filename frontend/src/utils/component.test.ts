...
describe('component graph', () => {
    ...
    it('should have proper sources', function () {
        ...
        // A and F both have no incoming connections (F->E is the only edge
        // touching F). X/Y/Z form a cycle and so are neither sources nor sinks.
        expect(allSources.map(c => c.name).sort()).toEqual(["A", "F"])
    });
