...
async function buildDepsData() {
  if (!store.hasData || !store.hasView("snippets")) return { nodes: [] as DepsNode[], edges: [] as DepsEdge[] }

  const rawEdges = await store.query<{ file: string; content: string }>(
  ...
  if (store.hasView("files")) {
    const fileComps = await store.query<{ name: string; component: string }>("SELECT name, component FROM files")
...
async function renderDepsGraph() {
  ...
  const { nodes, edges } = await buildDepsData()
