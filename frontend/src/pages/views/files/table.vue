...
// Discover all columns dynamically. store.query is async, so this cannot be
// a computed — populate a ref whenever the dataset changes.
const allFileColumns = ref<ColumnDef[]>([])
watch(() => store.hasData, async (hasData) => {
  if (!hasData || !store.hasView("files")) {
    allFileColumns.value = []
    return
  }
  const rawCols = await store.query<{ name: string }>("SELECT name FROM PRAGMA_TABLE_INFO('files') ORDER BY 1")
...
// Query files — async, so a watched ref rather than a computed.
...
watch(() => store.hasData, async (hasData) => {
  ...
  allFiles.value = await store.query<Record<string, any>>("SELECT * FROM files ORDER BY name")
}, { immediate: true })
