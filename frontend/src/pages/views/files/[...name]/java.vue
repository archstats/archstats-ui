...
const javaSnippets = ref<any[]>([])
watch([() => store.hasData, escapedPath], async ([hasData]) => {
  if (!hasData || !filePath.value) {
    javaSnippets.value = []
    return
  }
  try {
    javaSnippets.value = await store.query<any>(`
...
watch(() => store.hasData, async (hasData) => {
  if (!hasData) {
...
// store.query is async, so this cannot be a computed — populate a ref instead.
const resolvedOutgoingReferences = ref<{ className: string; file: string; component: string; roles: string[]; isInternal: boolean }[]>([])
watch([() => store.hasData, escapedPath, classToFileMap], async ([hasData]) => {
  if (!filePath.value || !hasData) {
    resolvedOutgoingReferences.value = []
    return
  }
  try {
    ...
    for (const imp of imports) {
      ...
      if (dest) {
        const isInternal = dest.component === (await store.query<any>(`SELECT component FROM files WHERE name = '${escapedPath.value}' LIMIT 1`))[0]?.component
        list.push({
          className,
          file: dest.file,
    ...
    resolvedOutgoingReferences.value = list
  } catch {
    resolvedOutgoingReferences.value = []
  }
}, { immediate: true })
...
// store.query is async, so this cannot be a computed — populate a ref instead.
const resolvedIncomingReferences = ref<{ className: string; file: string; component: string; roles: string[] }[]>([])
watch([() => store.hasData, declaredClasses, escapedPath], async ([hasData]) => {
  if (declaredClasses.value.length === 0 || !hasData) {
    resolvedIncomingReferences.value = []
    return
  }
  const list: { className: string; file: string; component: string; roles: string[] }[] = []
  try {
    for (const className of declaredClasses.value) {
    ...
    resolvedIncomingReferences.value = list
  } catch {
    resolvedIncomingReferences.value = []
  }
}, { immediate: true })
