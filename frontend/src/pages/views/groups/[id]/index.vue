...
const membersMetrics = ref<{ name: string; displayName: string; found: boolean; loc: number; health: any; hotspot: any; link: string }[]>([])
watch([() => dataStore.hasData, group], async ([hasData]) => {
  if (!group.value || !hasData) {
    membersMetrics.value = []
    return
  }

  if (group.value.type === 'component') {
    membersMetrics.value = group.value.members.map(member => {
    ...
    const filesList = await dataStore.query<Record<string, any>>(
