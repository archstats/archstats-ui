...
const gitCouplingData = ref<{ pair_1: string; pair_2: string; shared_commits: number }[]>([])
watch(() => dataStore.hasData, async (hasData) => {
  if (!hasData) {
    gitCouplingData.value = []
    return
  }
  try {
    gitCouplingData.value = await dataStore.query<{ pair_1: string; pair_2: string; shared_commits: number }>(
