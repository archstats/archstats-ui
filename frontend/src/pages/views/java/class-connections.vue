...
const allJavaClasses = ref<string[]>([])
watch(() => store.hasData, async (hasData) => {
  if (!hasData || !store.hasView("files")) {
    allJavaClasses.value = []
    return
  }
  const rows = await store.query<{ java_full_class: string }>(
    "SELECT DISTINCT java_full_class FROM files WHERE java_full_class != '' ORDER BY java_full_class"
  )
  allJavaClasses.value = rows.map((r) => r.java_full_class)
}, { immediate: true })

// store.query is async, so this cannot be a computed — populate a ref instead.
const classMetaMap = ref(new Map<string, { component: string; file: string }>())
watch(() => store.hasData, async (hasData) => {
  const map = new Map<string, { component: string; file: string }>()
  if (hasData) {
    const rows = await store.query<{ java_full_class: string; component: string; name: string }>(
