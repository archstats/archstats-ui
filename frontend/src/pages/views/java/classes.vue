<template>
  ...
  >
    <template #stats>
      <span>Classes <span class="text-neutral-800">{{ classes.size }}</span><span v-if="scope.isActive"> of {{ totalClasses }}</span></span>
    ...
    <template #visualizer>
      <EmptyState v-if="!hasTables" title="No Java class connections in this snapshot" text="The scan did not produce a class map and a java_class_connections_direct table." icon="braces"/>
...
import { frameworkStorageKey, loadRawClasses, rememberedFramework, shortName, type RawClass } from "~/utils/javaFacts"
...
const hasTables = computed(() => store.hasData && store.hasView("files") && store.hasView("java_class_connections_direct"))
const hasIndirect = computed(() => store.hasData && store.hasView("java_class_connections_indirect"))

const rawClassesQuery = useAsyncQuery<Map<string, RawClass>>(() => loadRawClasses((sql) => store.query(sql), (v) => store.hasView(v)), [], { initial: new Map<string, RawClass>() })
