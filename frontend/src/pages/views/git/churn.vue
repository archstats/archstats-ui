...
watch([() => store.hasData, searchQuery], async ([hasData]) => {
  if (!hasData) {
    churnData.value = []
    return
  }
  try {
    const raw = await store.query<ChurnRow>(
    ...
    churnData.value = q ? raw.filter(r => r.name.toLowerCase().includes(q)) : raw
  } catch { churnData.value = [] }
}, { immediate: true })
