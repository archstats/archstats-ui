...
// Component Hotspots
const componentHotspots = ref<{ component: string; commits: number; additions: number; deletions: number }[]>([])
watch([() => store.hasData, escapedName], async ([hasData]) => {
  if (!hasData) {
    componentHotspots.value = []
    return
  }
  componentHotspots.value = await store.query<{ component: string; commits: number; additions: number; deletions: number }>(
...
const hotspotScores = ref(new Map<string, number>())
watch([() => store.hasData, componentHotspots], async ([hasData]) => {
  if (!hasData || componentHotspots.value.length === 0) {
    hotspotScores.value = new Map<string, number>()
    return
  }
  const componentNames = componentHotspots.value.map(c => `'${c.component.replace(/'/g, "''")}'`).join(',')
  const rows = await store.query<{ name: string; codesmells__hotspot_score: number }>(
...
// Coupling Partners
const couplingPartners = ref<{ author_name: string; shared_components: number; commits: number }[]>([])
watch([() => store.hasData, escapedName], async ([hasData]) => {
  if (!hasData) {
    couplingPartners.value = []
    return
  }
  couplingPartners.value = await store.query<{ author_name: string; shared_components: number; commits: number }>(
