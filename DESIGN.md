...
4. **Export menu.** One download button in `actions` (⌘E), with Figure, Table and Document sections from whatever the view registered. One feedback pattern: "Copied" or "Saved", the inline error, and disabled items that say why. Table exports may add notes (a rollup rule per column) to the CSV preamble. Every figure keeps one contract, enforced in `utils/figure.ts` on the way out (PNG, SVG, pins, reports): it has a size and something drawn before it renders, and the image that comes out is not blank and, in the light rendering, has a light ground; otherwise it fails with a sentence the Export menu shows ("The chart has no size to capture…", "The figure came out blank…"). A chart registers its figure through `useSvgFigure` (ready only when its element is on screen, sized and drawn) or `register({ kind: "figure" })`, or says `figure: none` and why; a unit test holds every chart component to that, and another holds the palette to one light colour per dark one (`REMAP_PREFERRED` settles shared values). Only the chart is remapped to light, never the ground and footer written in light colours. `npm run check:figures` opens every chart view on a real snapshot with `wails dev` running and exports each figure in both appearances through the same code, and each table, failing on anything a report could not hold.
...
8. **Pin button and pool.** The bookmark is the only pin icon. Pins form a pool per workspace that every report draws on; the pool lists each pin with its kind, its status against the newest snapshot and the reports that use it.
...
**Report notebook (Evidence, 2026-09-24).** A three-pane notebook in the tool-window frame: reports and the open report's outline on the left (232px, ground), the report in the middle (a 760px reading column on surface, the title at 30px semibold, body at 15px/1.65), and the Pool | Cell tabs in the inspector. Prose is Markdown that renders when the caret leaves it and shows its source in the same type where the caret is; "## ", "- ", "1. " and "> " convert as typed, ``` and --- on Enter. Evidence is a cell: a hairline figure with the number (Table 2, Figure 1), an editable title and caption, the frozen output, and a mono provenance line; its run gutter sits outside the column (▶ and the snapshot date), orange only when the cell ran on another snapshot. A row's handle (+ and grip) appears on hover at the column's left edge. One insert palette serves "/", the +, and the A and B keys: groups Pins, Tables, Query and Text, each row previewed on the right (a table runs six rows on the report's snapshot). A pin can also be dragged in from the pool, landing at an orange drop line labelled "Insert pin here". Command mode is the notebook's: Esc selects the block, arrows or J/K move, Enter edits, ⇧↵ runs, Backspace or D D removes, ⌘Z undoes across blocks. ⌘/ shows the whole report as Markdown, cells as fenced `archstats` blocks. Every view's Export menu offers "Add to report…" for its figures, tables and documents, which opens the import sheet: a wide sheet (1160px) with the report and the landing spot on the left (the report's outline, the spot drawn as an orange line labelled "Lands here"), a lite editor in the middle (the report's blocks around the spot at 55% opacity, the incoming material in a section with an orange inset line and an "Adding" legend, "Write above" and "Write below" opening live-Markdown paragraphs), and the trim on the right (rows and columns of a table, light or as shown for a figure, the provenance it keeps). The footer says what will happen in one sentence ("Adds Table 3 under “Coupling” in “Audit”, with 1 paragraph of yours") beside Cancel, Add and open, and Add (⌘↵). PDF opens a near-full-window preview first (⌘⇧E): the laid-out file itself in the system's PDF view, a header with pages, page size (A4 | Letter) and file size, "Open in Preview" and Save PDF… (⌘S), which saves exactly the bytes shown; a breathing page skeleton while Go lays it out, and the error with Try again if it cannot. The PDF is laid out in Go (A4 or Letter, Inter and JetBrains Mono embedded, a 24mm orange rule under the title, numbered tables that repeat their header across pages, provenance under every cell, page n of m in the footer).

**Cycles as tangles (2026-09-24).** The Cycles view is built around tangles (strongly connected sets of two or more components), not the engine's list of shortest cycles, which overlap on a few imports (Broadleaf: 531 listed cycles in 18 tangles). A strip of tangles to scale, each a segment as wide as its components (the open one accent-500, search matches accent-200), sits over a readout: Tangle n, the shared prefix its names leave out ("in org.broadleafcommerce.common"), components, lines, levels, listed cycles and groups crossed, with Pin and Save as group. A status line under it always holds the cuts' effect ("4 cuts: 6 of 61 components out of any tangle · 3 tangles left (50, 3, 2) · Undo cuts", or "Nothing cut yet…"), so cutting never moves the drawing. The tangle is ordered so the imports running backwards weigh least (weighted Eades–Lin–Smyth, then sifting) and laid out in levels by longest path; an import to a later level runs with them, everything else against, and the ones against are what make every cycle. Levels draws them top to bottom on alternating neutral-50 bands labelled LEVEL n: components as dots sized by lines and filled in their lens group's colour, an accent-200 halo while still tangled, hollow once freed; names in mono under the dot; imports with the levels as neutral hairlines; imports against them as accent arcs out to the right (a dip below the row within one level), dashed and muted once cut; hover or selection lights a component's imports and dims the rest; zoom, fit and pan. Matrix draws the same order as rows and columns (row imports column) with each level a neutral-50 block on the diagonal, imports with the levels neutral above it, against them accent below it, a cut one a dashed outline; it is the default past 60 components or 12 per level. The Guide tab, open by default, walks the plan in plain words. Before it starts it says what the drawing is ("These 29 components are knotted together: follow the imports from any one of them and you reach all the others…", with inline swatches for the grey and orange imports) and that cutting changes nothing in the code. Each step then names the import (`from → to` in mono), says the loop it closes ("… and annotations leads back to it through crud"), states what cutting it does from where the cuts stand now, not from the plan's assumption, on an accent-50 panel ("Cutting it frees 2 components: exceptions, json"; "splits the knot in 20 and 9"; "frees no one yet; it opens the way for the cuts after it"), and shows the import lines themselves, file name, line number and the source line read from file_contents, with co-changes as a fact. "Cut it, next" (→ or ↵) cuts and moves on, skipping cuts that earlier ones made unnecessary; Skip (S) and Back (←). While a step is open the drawing zooms to its loop (0.52s cubic ease-out, none with reduced motion), dims everything else to 7%, rings the loop's components in ink, draws its imports in ink and the import to cut as marching orange dashes with an accent pill at the arc's middle ("Cut here · frees 2"); a cut's freed components flash an expanding accent ring once. A progress bar reads components free out of the tangle. The last card says the knot is undone (or how many are still knotted after skips) and offers the sandbox, Start over and the next knot. Clicking a planned cut in the drawing opens it in the guide. The All cuts tab is the untangling: one sentence ("39 cuts, 76 imports in 52 files, leave no cycle in this tangle. The first 22 free half of it."), a step chart of components still tangled cut by cut with the current point marked (click to jump), a range to apply the first k, and the cuts in order, each with a checkbox, its imports, files and co-changes, and what it does in accent ("frees 3") or muted ("splits", "narrows"); ↑↓ move and Space cuts. Each step takes the cut that lowers the sum of squared tangle sizes most, then the fewest imports; it is stated as a plan, not the only one. "Try n cuts in the sandbox" hands the cuts to Connections' what-if. Selection shows an import's plan step and effect, imports, files, co-changes and listed cycles, and where the imports are (file name, its folder below, line ranges with snippet popovers), or a component's level, lines, imports in and out, and its imports against the levels. Cycles lists the engine's shortest cycles inside the tangle, standing first, struck through with "broken by cut n" once cut, and lights one's edges in the drawing when chosen. There is no severity score.

**SQL editor (SQL console and notebook SQL cells, 2026-09-24).** One component, `SqlEditor`, for every place SQL is written: a native textarea (selection, ⌘Z and input methods stay the system's) over a layer that draws the same text in JetBrains Mono 12.5/20 with its meaning. Keywords violet-700 medium, functions blue-700, strings green-700, numbers amber-800, comments neutral-400 italic, tables neutral-950 semibold, metric columns neutral-900 with a dotted neutral-300 underline (they have a definition to show), aliases and CTE names accent-700, operators neutral-500; the pair of a bracket beside the caret on accent-200, the caret's line on a neutral-100 band, the caret accent-600. Problems are a red wavy underline on the token and a red line number: what can be told before running (no such table, with the likely one: "did you mean components?"; a column its table lacks; an unclosed string), held back while the caret is still in the word, and SQLite's own error placed on the token it names. A 26px line under the text says what the caret is on: the function's signature with the current argument underlined in accent, a problem (click to jump), a metric's name and definition, a table's columns, else the keys (⌃Space suggests, ⌘↵ runs, ⇧↵ in the notebook, ⌘/ comments a line) and Ln, Col. Suggestions open as a word is typed, after a dot, after FROM or JOIN, and inside a quoted comparison, and follow the clause: tables after FROM (with CTEs), an alias's columns after its dot, the columns of the tables in scope (CTE and subquery outputs included, output names in ORDER BY), functions with their signatures, the clause's keywords; a column that is a keyword is quoted on insert. Metrics are found by what they are called ("health" finds codesmells__code_health), letters scattered over many fragments are not a match, and the matched letters are bold. Inside quotes the list is the column's values in the table it resolves to, most common first with row counts, long ones kept at both ends. The popup (360px, 460px for values) sits under the caret with a 260px documentation pane on neutral-50: title, definition, and the id, type and tables in mono. Brackets and quotes close themselves, Enter keeps the indent (one deeper after an opening bracket), Tab indents lines. The console adds a 40px line-number gutter and a schema sidebar with a filter over tables, columns and metric names, column types, and Query (or Insert) on a table's row.

**Report templates (Evidence, 2026-09-24).** New report opens a template gallery (1120px sheet) instead of an empty page. Left (284px, ground): Start (Blank report), "For <workspace>" (ecosystem templates the snapshot shows, each with its evidence in accent: "Found: 750 Spring beans, 157 JPA entities"), General (ten), Yours (saved templates, delete on a second click), and a collapsed "Other ecosystems (n)". Right: a band on ground with the template's name, audience and one-sentence summary, a tally ("Writes 6 sections: 7 paragraphs counted from the snapshot, 1 table, 3 figures to add from the views and 2 prompts for your reading.") and what it leaves out and why ("Leaves out Tests (files have no roles)"), with a component field for templates that take one; below, the report as it will open, in the notebook's reading type, its facts already counted. Footer: the report's name and Create report (⌘↵; ↑↓ move through templates). A template writes four kinds of block. *Computed paragraphs* (reading cells) are facts counted the way the views count them, set in the body type with bold numbers and code-span names; a 2px dotted rule in the margin (solid orange when selected) and a mono "computed" label in the run gutter mark them as counted, not written; they re-run like any cell and say "moved" when a number did; "Write as my own" in the Cell pane turns one into a paragraph that no longer re-runs. *Prompts* are empty paragraphs whose placeholder, in italic neutral-500 beside a 2px neutral-200 rule, says what the writer's reading should say there; they never print. *Tables and queries* run on create (a query's SQL shows only while its cell is selected). *Slots* are figures and tables the template asks for: a hatched card with the number it will take ("Figure 1"), a sketch of a chart or rows, "Add it from Connections: Graph, by group." and Open Connections. Opening one carries the slot into the view: a bar under the toolbar ("Filling Figure 1, Dependency structure in “Spring review”. Set it as graph, by group, then add it.") with Add this view (disabled with "Nothing drawn here to add yet" until a figure is drawn), Back to report and ×. The import sheet then lists the report's slots as places ("Fill Figure 1 · Dependency structure", orange icon, "Fills this slot"), picks the one being filled (or one for the same view) by default, and offers Fill and Fill and return. *Taking* fills slots without the hunt: a slot names its ask in the view's own settings (chips such as "Shown as Graph", "Level By group", read back from the route by `slotSettings`), and "Take it from Connections" (on the slot, in the Cell pane, "Take 3 figures" in the toolbar, "Resume taking · 2 left" after a pause, and a "Then take the 3 figures from their views" checkbox in the template gallery, on by default) opens each view set as asked, waits until it has drawn and settled (1.4s for a figure, 0.3s for a table; up to 15s, then the bar says the view drew nothing to take), and opens the import preview on it. In that run the preview's title counts ("Taking 2 of 3") beside a row of 16×3px progress segments, and its right column opens with Asked for: the slot's title and number, then each setting as asked with a quiet check, or an orange alert and "taken as Components" where the capture differs, "Take it as asked" to go again; the footer is Stop, Skip and Fill and next (Fill and finish on the last; ⌘↵). Esc pauses the run; the bar then offers Take as asked, Add this view, Skip and Back to report. A view with several exports hands the slot the last one registered that has something in it (the grain's own table, not the page's empty one). Slots and prompts are left out of the PDF and Markdown; the PDF preview's header says how many are still to add. Save as template… (a report's overflow menu) is a 480px dialog: name, what it is for, "Turn my paragraphs into prompts" (on by default), and a checklist of what comes with it; results never do, and captures and pins become slots for their views.
