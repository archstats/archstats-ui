<template>
  <Card class="w-2/3 mt-24 bg-white">
    <h1 class="text-lg ">Walk <span
        ...
        :class="{ 'text-tertiary-500': column == 'to','border-b-tertiary-500': column=='to',  'text-secondary-500': column == 'from', 'border-b-secondary-500': column=='from'}"
        @click="toggleDirection">{{ column }}</span> <span
        class="font-mono font-normal">{{ component }} <Icon class="inline"
                                                            :class="{ 'text-tertiary-500': column == 'to', 'text-secondary-500': column == 'from'}"
    ...
    <div class="mt-8 mb-12 flex">
      ...
      <input class="w-full px-4 py-2 bg-gray-100  box-border outline-tertiary-500 outline-1" v-model="searchText"
