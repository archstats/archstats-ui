<template>
  <tr  :class="{'border-b-2':!expanded,'font-semibold': userSelectedPositionInPath == nr-1, 'opacity-40': nr-1 > userSelectedPositionInPath }">
    ...
    <td class="font-semibold pr-4">{{ nr }}</td>
    <td class="font-mono cursor-pointer hover:underline" @click="select">{{
        segment.currentComponent
      }}
    </td>
    <td class="font-semibold pl-4 flex text-blue-500"
        :class="{'text-red-500': segment.relationship == 'depends on'}">
