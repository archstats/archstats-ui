<template>
  <div class="flex   text-white rounded overflow-clip">
    <button class="transition-all w-full px-4 py-1 bg-sky-500 overflow-ellipsis outline-0" @click="executeAction(selectedAction)">{{selectedAction.name}}</button>
    <select v-model="selectedAction" class="w-4 bg-sky-500 rounded-r outline-0 " >
