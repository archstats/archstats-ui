...
// Module-level, non-reactive reference to avoid Vue wrapping
// DbWorker internals (nextId, pending Map) in reactive proxies,
// which would cause infinite watchEffect re-triggers on every query.
let _dbInstance: DbWorker | null = null;
...
export const useDataStore = defineStore('data', {
    ...
    getters: {
        ...
        hasData: state => _dbInstance !== null && state._initialized,
        hasView() {
    ...
    actions: {
        async query<T = any>(qryString: string): Promise<T[]> {
            ...
            if (!_dbInstance) {
                console.debug("No database available in store. Returning empty list.")
                return []
            }
            return _dbInstance.query<T>(qryString);
        },
        ...
        async setViews(views: Uint8Array) {
            // Terminate previous worker if exists
            if (_dbInstance) {
                _dbInstance.terminate();
            }
            _dbInstance = new DbWorker();
            await _dbInstance.init(views);
