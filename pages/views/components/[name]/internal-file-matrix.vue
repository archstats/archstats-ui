    ...
    <div v-if="component" class="flex flex-col gap-6 animate-fade-in">
      ...
      <section class="bg-gradient-to-r from-slate-50 to-slate-100/50 border border-slate-100 rounded-3xl p-6 shadow-xs flex flex-col gap-2">
        ...
        <h2 class="text-xl font-bold text-slate-800 tracking-tight flex items-center gap-2">
          <span>Internal File Coupling Matrix</span>
          <span class="px-2 py-0.5 rounded-full text-[10px] font-extrabold tracking-wider uppercase bg-emerald-50 border border-emerald-200 text-emerald-700 font-mono">
            Intra-Component
          </span>
        </h2>
        <p class="text-xs text-slate-400 leading-relaxed max-w-2xl">
          Trace structural, logical, and name relationships among files within this component. Helps highlight internal clusters (like connecting entities to their repos or controllers).
        </p>
      </section>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="bg-white border border-slate-150 p-4 rounded-2xl shadow-3xs flex flex-col justify-between gap-2">
          <div class="flex items-center justify-between">
            <span class="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Internal Semantic Ties</span>
            <span class="w-2 h-2 rounded-full bg-blue-500"></span>
          </div>
          <div>
            <span class="text-2xl font-black text-slate-800 font-mono">{{ semanticTiesCount }}</span>
            <span class="text-[10px] font-bold text-slate-400 ml-1">pairs</span>
          </div>
          <p class="text-[9px] text-slate-450 leading-normal">
            Internal file pairs sharing high linguistic similarity (Jaccard &ge; 0.5) in their basenames.
          </p>
        </div>
        ...
        <div class="bg-white border border-slate-150 p-4 rounded-2xl shadow-3xs flex flex-col justify-between gap-2">
          <div class="flex items-center justify-between">
            <span class="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Internal Git Co-Changes</span>
          ...
          <p class="text-[9px] text-slate-450 leading-normal">
            Internal file pairs modified together in the same commits (excluding bulk checkouts).
          </p>
        </div>
        ...
        <div class="bg-white border border-slate-150 p-4 rounded-2xl shadow-3xs flex flex-col justify-between gap-2">
          <div class="flex items-center justify-between">
            <span class="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Internal Graph Connections</span>
            <span class="w-2 h-2 rounded-full bg-emerald-500"></span>
          </div>
          <div>
            <span class="text-2xl font-black text-slate-800 font-mono">{{ graphConnectionsCount }}</span>
            <span class="text-[10px] font-bold text-slate-400 ml-1">direct ties</span>
          </div>
          <p class="text-[9px] text-slate-450 leading-normal">
            Internal file pairs directly importing or exporting each other in the dependency graph.
          </p>
        </div>
      </div>
      ...
      <section class="bg-white border border-slate-100 rounded-3xl p-6 shadow-xs flex flex-col gap-6">
        ...
        <div class="flex flex-col gap-4 border-b border-slate-100 pb-4">
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <h3 class="text-xs font-black uppercase tracking-widest text-slate-400">Internal File Relationships</h3>
          ...
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="border-b border-slate-100 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                <th class="py-3 px-4">File A</th>
                <th class="py-3 px-4">File B</th>
            ...
            <tbody class="divide-y divide-slate-50 text-xs font-semibold text-slate-700">
              ...
              >
                <td class="py-3 px-4 max-w-[220px] truncate font-mono text-[11px] text-slate-550">
                  <button 
...
const semanticTiesCount = computed(() => {
  return internalRelations.value.filter(r => r.linguisticSimilarity >= 0.5).length
})

const gitCoChangesCount = computed(() => {
  return internalRelations.value.filter(r => r.gitCoChanges > 0).length
})

const graphConnectionsCount = computed(() => {
  return internalRelations.value.filter(r => r.pathDistance === 1).length
})
