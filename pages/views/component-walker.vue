  ...
  <Splitpanes>
      ...
      <ComponentCardList :components="dependents" @component-selected="walkTo('is depended on by', $event)">
        <template #header>
          <div class="p-4 text-center">
            <h3 class="text-xl text-archstats-tertiary-500 mb-4 font-semibold">Afferent Coupling</h3>
            <p class="text-sm"><span class="font-mono text-xs font-bold text">{{ selectedComponent.name }}</span><br>
              is referenced
              <span class="text-archstats-tertiary-500 font-semibold">{{ totalReferencesAfferent }}</span> times in  <span class="text-archstats-tertiary-500 font-semibold">{{ selectedComponent['afferent_couplings'] }}</span> files by <span
                  class="font-semibold text-archstats-tertiary-500">{{ dependents.length }}</span> other components.</p>
          </div>
        </template>
      </ComponentCardList>
    </Pane>
      ...
      <Splitpanes horizontal>
        <Pane size="80">
          ...
          <div class="flex flex-col flex-shrink-0 h-full  align-middle justify-center items-center">
            ...
            <Card class="self-center relative">
              ...
              <div class="absolute top-1/2 -right-40 font-semibold text-archstats-secondary-500 text-center ">
                <h2>Dependencies</h2>
              </div>
              <div class="absolute top-1/2 -left-40 font-semibold text-archstats-tertiary-500">
                <h2>Dependents</h2>
              </div>
            </Card>
          </div>
        ...
        <Pane size="20" min-size="2" max-size="90" class="bg-gray-100 text-sm flex-grow   rounded-none flex flex-col">
          ...
          <div class="bg-gray-200 py-2 px-2 flex justify-between">
            <h3 class="font-semibold self-center text-archstats-primary-900">Path</h3>
    ...
    <Pane size="25" min-size="1" v-if="selectedComponent">
      <ComponentCardList :components="dependencies" @component-selected="walkTo('depends on', $event)">
        <template #header>
          <div class="p-4 text-center">
            <h3 class="text-xl text-archstats-secondary-500 mb-4 font-semibold">Efferent Coupling</h3>
            <p class="text-sm">
              <span class="font-mono text-xs font-bold text">{{ selectedComponent.name }}</span><br> has
              <span class="text-archstats-secondary-500 font-semibold">{{ totalReferencesEfferent }}</span> references in <span class="text-archstats-secondary-500 font-semibold">{{selectedComponent['efferent_couplings']}}</span> files to <span
                class="font-semibold text-archstats-secondary-500">{{ dependencies.length }}</span> other components.</p>
          </div>
        </template>
      </ComponentCardList>
    </Pane>
